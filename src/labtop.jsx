...
activebutton =()=>{
        for(let i = 0 ; i< Math.ceil(this.props.laptop.length /8 ) ; i++){
...
next_previos = ()=>{
        if(this.getdata().length !== 0 ){
           ...
           var nopage = Math.ceil(this.props.laptop.length / 8)
                ...
                if(+this.props.match.params.id > Math.ceil( this.props.laptop.length /4 )){this.props.history.push("/home")  
                }
            }
}
getdata = ()=>{
    var colnedproducts = [...this.props.laptop] ;
    var productslist = []
                ...
                <ul className="pagination" >
                        <li className="page-item-prev ">
                            <button className="btn prevbutton" id="prevbutton">
                                <Link className="page-link" to={`/home/labtop/${(+this.props.match.params.id)-1}`}>Previous</Link>
                            </button> 
                        </li>
                        {this.props.noLabtoppage.map(ind=>{
                             return(
                                   <li className="page-item" key={ind}>
                                       <Link to={`/home/labtop/${ind}`} className="page-link" >{ind}</Link>
                                   </li>
                        )})}
                        <li className="page-item-next "  >
                            <button className="btn" id="nextbutton">
                                <Link className="page-link"  to={`/home/labtop/${(+this.props.match.params.id || 1 ) +1}`}>Next</Link>
                            </button> 
                        </li>        
                    </ul>
                </nav>
